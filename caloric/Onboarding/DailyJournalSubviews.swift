...
struct WorkoutsCard: View {
    ...
    let manualWorkouts: [JournalStore.ManualWorkout]
    let onAddManual: () -> Void
    let onRemoveManual: (UUID) -> Void
    let cardBackground: AnyView
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack(spacing: 12) {
                ZStack {
                    Circle().fill(Theme.segEAT.opacity(0.15)).frame(width: 32, height: 32)
                    Image(systemName: "dumbbell.fill")
                        .font(.system(size: 14))
                        .foregroundStyle(Theme.segEAT)
                }
                Text(language == "de" ? "Workouts" : "Workouts")
                    .font(.poppins(size: 16, weight: .semibold))
                    .foregroundStyle(Theme.textPrimary)
                Spacer()
            ...
            VStack(spacing: 10) {
                ...
                ForEach(workouts) { w in
                    workoutRow(name: w.activityType.name, source: w.sourceName, icon: "applewatch")
                }
                
                // Manual Workouts
                ForEach(manualWorkouts) { mw in
                    HStack {
                        workoutRow(name: mw.name, source: language == "de" ? "Manuell" : "Manual", icon: "square.and.pencil")
                        
                        Button { onRemoveManual(mw.id) } label: {
                ...
                if workouts.isEmpty && manualWorkouts.isEmpty {
                    Text(language == "de" ? "Keine Workouts für diesen Tag erfasst." : "No workouts recorded for this day.")
                        .font(.poppins(size: 13, weight: .regular))
                        .foregroundStyle(Theme.textSecondary)
    ...
    private func workoutRow(name: String, source: String, icon: String) -> some View {
        HStack(spacing: 12) {
